...
class Node:
	def __init__(self, inDf: pd.DataFrame, groupName: str, colWidths: List[int], depth: int = 0):
		self.depth = depth
		...
		self.leaves: List[str] = None
		myColWidth = self._createChildren(df)

		self._absorbFirstChild()

		if len(self.children) > 0:
			self.topContentLen = self.children[0].topContentLen + 1
			# If we've gotten to here we have already absorbed the first child, which fills out colWidths
			...
			self.topColWidths = self.colWidths
			self.bottomColWidths = [*colWidths, myColWidth]
	...
	def _createChildren(self, df: pd.DataFrame):
		...
		# However, if there is only a single column there is nothing to group, convert the series to a list of strings
		# That is a leaf node (no children)
		groups: List(tuple) = []
		if len(df.columns) > 1:
			...
			self.leaves = list(series)

		self.colWidths[-1] = myColWidth
		for groupName, group in groups:
			self.children.append(Node(group, groupName, self.colWidths, self.depth+1))
		return myColWidth
	...
	def _absorbFirstChild(self):
		# In the case of a leaf node, just absorb the first leaf line into the first line
		if len(self.leaves) > 0:
			self.firstLine.append(self.leaves[0])
	...
	def render(self) -> List[Line]:
		...
		for leafCell in self.leaves:
			renderedLines.append(Line([leafCell], self))
