...
#			self.effChildren.append(pendingLeafNode)
	...
	def render(self):
		...
		if not self.isExpanded and len(self.effChildren) > 1:
			renderedLines.append(Line(self.colWidths, [self.childColName], self, elDecorators = Line.HEADER))

			contentLine = [Chars.UP_ARROW + ' ' + self.myContent, str(self.colSummary)]
			elDecorators = [Line.FOCUSED if self.isFocused else Line.NORMAL, Line.NORMAL]
			renderedLines.append(Line(self.colWidths, contentLine, self, elDecorators = elDecorators))
		...
		if len(renderedLines) > 0:
			...
			for line in renderedLines:
				...
				if Line.HEADER in line.elDecorators and not headerAdded:
					line.insertContentCell(self.childColName, Line.HEADER)
					headerAdded = True
				elif not Line.HEADER in line.elDecorators and not contentAdded:
					...
					line.insertContentCell(contentLine, Line.FOCUSED if self.isFocused else Line.NORMAL, self)
					contentAdded = True

			# If we didn't a header, insert it at the top (first non hline index)
			if not headerAdded:
				for c in range(len(renderedLines)):
					if isinstance(renderedLines[c], Line):
						renderedLines.insert(c, Line(self.colWidths, [self.childColName], self, elDecorators = Line.HEADER))
						break
...
#			renderedLines[c].insertContentCell(self.childColName, Line.HEADER)
...
#			else:
#				contentLine = self.myContent
#			renderedLines[c+1].insertContentCell(contentLine, Line.FOCUSED if self.isFocused else Line.NORMAL, self)
...
#		return
	...
	def render(self):
		...
		if self.baseHeaderLine is not None and len(self.baseHeaderLine) > 0 and not self.headerHidden:
			renderedLines.append(Line(self.effColWidths, self.baseHeaderLine, self, elDecorators = Line.HEADER))
		...
		if self.baseContentLines is not None and len(self.baseContentLines) > 0:
			for r in range(len(self.baseContentLines)):
				...
				if r == self.focusedLineIdx and self.isFocused:
					elDecorators = [Line.FOCUSED if c == self.focusedCellIdx else Line.NORMAL for c in range(len(line))]
				else:
					elDecorators = Line.NORMAL
				renderedLines.append(Line(self.effColWidths, line, self, elDecorators = elDecorators))
